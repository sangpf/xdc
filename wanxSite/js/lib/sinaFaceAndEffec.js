...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			var eTop = target.offset() + target.height() + 15;
			var eLeft = target.offset() - 1;
			...
			$('#emotions').html('<div style="float:right"><span id="prev">&laquo;</span><span id="next">&raquo;</span></div><div class="categorys"></div><div class="container"></div>');
		...
		function showCategorys(){
			...
			cat_page = page;
			for(var i = page * 4; i < (page + 1) * 4 && i < categorys.length; ++i){
				$('#emotions .categorys').append($('<span>' + categorys[i] + '</span>'));
			}
			$('#emotions .categorys span').click(function(){
				showEmotions($(this).text());
			});
			$('#emotions .categorys span').each(function(){
		...
		function showEmotions(){
			...
			for(var i = 0;  i < emotions[category].length; ++i){
				$('#emotions .container').append($('<span title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></span>'));
			}
			$('#emotions .container span').click(function(){
				$('.comInner').insertText($(this).attr('title')).blur();
				$('#submit').css('color','#ff7b00');
				// $('#emotions').remove();
			});
			$('#emotions .container span').width(30).height(30);
			$('#emotions .categorys span.current').removeClass('current');
			$('#emotions .categorys span').each(function(){
