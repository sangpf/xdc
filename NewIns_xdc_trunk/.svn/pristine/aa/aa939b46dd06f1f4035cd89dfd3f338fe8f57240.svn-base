...
//		log.info("==============================>> 简单测评模型 计算  输出结果 :"+assessResult.toString());
	...
	private AssessResult Calculation_Assess_model_multi_combination(Map<String, Object> dataMap,AssessResult assessResult){
		...
		while(iterator.hasNext()){
			...
			log.info("======================>> 将得分 和最大分 进行非负化 , 得分非负化 :"+user_score + " 最大区间分非负化 :"+max_score);
			// 用户得分 百分化
			double user_score_div = MathUtil.div(user_score, max_score, 10) * 100;  
			
			log.info("========================>>多维度组合输出题 , 维度id :"+ dimension +" 用户得分百分化值 : "+user_score_div);
		...
		log.info("===========================>> 多维度组合输出模型 , 所有维度得分百分化, 之和 :"+ user_score_div_totle);
		double percentageScore_double = MathUtil.div(user_score_div_totle, user_score_div_totle_size);
		int percentageScore_int = MathUtil.getInt(percentageScore_double);
		
		log.info("===========================>> 多维度组合输出模型 , 所有维度得分百分化, 计算double平均值 , :"+ percentageScore_double);
		log.info("===========================>> 多维度组合输出模型 , 所有维度得分百分化, 计算int平均值 , :"+ percentageScore_int);
		assessResult.setPercentageScore(percentageScore_int);
		...
		if(get_combination_relation_Bydimension_combination != null && get_combination_relation_Bydimension_combination.size()>0){
			...
			log.info("=====================>> 根据关联关系 :"+combination_relation+",从数据库中查询出测评结果 . ");
