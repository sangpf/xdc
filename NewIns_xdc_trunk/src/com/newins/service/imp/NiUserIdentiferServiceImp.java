...
import com.newins.dao.NiUserDeviceMapper;
...
import com.newins.dao.NiUserPostalMapper;
...
public class NiUserIdentiferServiceImp implements NiUserIdentiferService{
	...
	//条件查询
/*	public List<NiUserIdentifer> selectList(NiUserIdentifer record) {
		List<NiUserIdentifer> selectList = niUserIdentiferMapper.selectList(record);
		return selectList;
	...
	public void updateUserInfo(String detailUserInfo,HttpSession session) {
		if(detailUserInfo!=null){
			...
			if(fromObject1.toString().contains("errorCode")){
				...
				Integer wanxUserid = fromObject1.getInt("userid");    
...
//				String wanxOpenid = (String) fromObject1.get("openid");   //openid
				...
				session.setAttribute("schoolId",fromObject1.getInt("school_id"));
				//获取地区信息
				String cityId = "";
				...
				if(wanxUserid != null){
					...
					if(userid!=null){
						...
						log.info("用户标识信息表未创建,session中未获取出新洞察userid");
					}
	...
	/**
	 * 更新玩校userId ,返回新洞察userId   老接口
	 */
	@Transactional
	public int mapUser_bak(String detailUserInfo,UserContext userContext,HttpSession newSession,HttpServletRequest request) {
		JSONObject fromObject1 = JSONObject.fromObject(detailUserInfo);
		log.info("玩校返回的信息===========================>>"+fromObject1.toString());
		Integer userid = null;
		if(fromObject1.toString().contains("errorCode")){
			...
			String wanxOpenid = (String) fromObject1.get("openid");   //openid
			int wanxuserId = fromObject1.getInt("userid");   // 玩校userid
			String bindPhone =  (String) fromObject1.get("mobile");  //手机号
			...
			if(niUserIdentifer != null){
				...
				NiUserIdentifer niUserIdentifer2 = new NiUserIdentifer();
				//niUserIdentifer2.setWanxuserid(wanxuserId);
				niUserIdentifer2.setBindPhone(bindPhone);
